    ...
    def camera_handler_callback(self, msg):
        ...
        if(self.processor == 0):
            ...
            if(arr[1] == 1):
                ...
                if(self.node_arr[1] != 0):
                    launch = self.node_arr[1] 
                    launch.shutdown()
                    self.node_arr[1] = 0
            
            # Customizing Camera 3
            
            if(arr[2] == 1):
                if(self.node_arr[2] == 0):
                   launch = self.get_launch_node(self.camera1)
                   launch.start()
                   self.node_arr[2] = launch
            else:
                if(self.node_arr[2] != 0):
                    launch = self.node_arr[2] 
                    launch.shutdown()
                    self.node_arr[2] = 0

            # Customizing Camera 4

            if(arr[3] == 1):
                if(self.node_arr[3] == 0):
                   launch = self.get_launch_node(self.camera1)
                   launch.start()
                   self.node_arr[3] = launch
            else:
                if(self.node_arr[3] != 0):
                    launch = self.node_arr[3] 
                    launch.shutdown()
                    self.node_arr[3] = 0
...
if __name__ == '__main__':
    ...
    print("0 - Jetson")
    print("1 - Raspberry Pi 2")
    print("2 - Raspberry Pi 1")
    CameraNode()
    rospy.spin()
