...
def send_usr_data(name, origin, result):
    db = firebase.database()
...
def get_usr_data(name):
    db = firebase.database()
    try:
        users = db.child("results").child(name).get()
        origin_result = []
        inference_result = []
        # for user in users.each():
        #     origin_result.append(json2im(user.val()["origin"]))
        #     inference_result.append(json2im(user.val()["result"]))
        return origin_result, inference_result        
    except:
        return False


print(get_usr_data("saatwik"))
